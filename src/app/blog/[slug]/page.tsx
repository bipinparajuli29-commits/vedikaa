    ...
    <article className="min-h-screen pt-24">
      <div className="relative aspect-[21/9] max-h-[520px] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          ...
          <h1 className="font-serif text-[clamp(1.8rem,4vw,3.5rem)] font-light text-white leading-[1.2] max-w-3xl">{post.title}</h1>
      ...
      <div className="max-w-3xl mx-auto px-6 lg:px-0 py-16">
        ...
        {post.content && (
          <div className="prose-vedika" dangerouslySetInnerHTML={{ __html: post.content }} />
        )}
        <div className="mt-16 p-8 border border-gold/20 bg-gold/[0.03] text-center">
          ...
          <h3 className="font-serif text-2xl font-light text-white mb-3">Let us Capture Your Love Story</h3>
          <p className="text-[0.82rem] text-white/45 mb-6">Book a free consultation with Vedika Weddings.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          {related.map((p) => (
            <Link key={p.slug} href={`/blog/${p.slug}`} className="group border border-white/8 hover:border-gold/30 transition-colors overflow-hidden">
              <Image src={p.image} alt={p.title} width={400} height={250} className="w-full aspect-video object-cover" style={{ filter: 'brightness(0.7)' }} />
              <div className="p-5">
